...
namespace RadCms.Helpers
{
    ...
    {
        static CmsHelper(){
            _site = ConfigurationManager.AppSettings["site"].ToString();
            _siteRoot = ConfigurationManager.AppSettings["site_url"].ToString();
            _cmsRoot = ConfigurationManager.AppSettings["cms_url"].ToString();
            _adDomain = ConfigurationManager.AppSettings["ad_domain"].ToString();
            _emailDomain = ConfigurationManager.AppSettings["email_domain"].ToString();
            _helpPage = ConfigurationManager.AppSettings["help_page"].ToString();
            _supportEmail = ConfigurationManager.AppSettings["support_email"].ToString();
        }
        ...
        private static string _helpPage;
        ...
        private static string _cmsRoot;
        public static string CmsRoot
        {
            get
            {
                return _cmsRoot;
        ...
        public static string FixUrl(string url)
        {
            ...
            {
                string host = GetWebHost().ToLower();

                if (url.ToLower().StartsWith(host))
                {
                    string newurl = url.Substring(host.Length);

                    return newurl.Length == 0 ? "/" : newurl;
                }
                else
                {
                    return url;
